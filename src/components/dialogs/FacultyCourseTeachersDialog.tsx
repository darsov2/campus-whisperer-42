...
export function FacultyCourseTeachersDialog({
  ...
  const updateRole = (teacherId: string, faculty: string, role: FacultyCourseTeacher["role"]) => {
    setTeachers(
      teachers.map((t) =>
        t.id === teacherId && t.faculty === faculty ? { ...t, role } : t
      )
    );
  };
  ...
  // Group assigned teachers by faculty
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Faculty Teachers - {courseCode}
          </DialogTitle>
          <DialogDescription>
            Assign which teachers teach <strong>{courseName}</strong> at each
            faculty. This is independent from programme-level assignments.
          </DialogDescription>
        </DialogHeader>
            ...
            {Object.keys(teachersByFaculty).length === 0 ? (
              <div className="text-center py-6 border border-dashed rounded-lg bg-muted/20">
                ...
                <p className="text-muted-foreground text-sm">
                  No faculty-level teachers assigned
                </p>
              </div>
            ) : (
              Object.entries(teachersByFaculty).map(([fac, facTeachers]) => (
                <div key={fac} className="space-y-2">
                  ...
                  {facTeachers.map((teacher) => (
                    <div
                      key={`${teacher.id}-${teacher.faculty}`}
                      className="flex items-center gap-3 p-3 bg-card border rounded-lg ml-5"
                    >
                      ...
                      <Select
                        value={teacher.role}
                        onValueChange={(value: FacultyCourseTeacher["role"]) =>
                          updateRole(teacher.id, teacher.faculty, value)
                        }
                      >
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        onClick={() => removeTeacher(teacher.id, teacher.faculty)}
