...
export function BulkRuleAssignmentDialog({
  ...
  const addGroup = () => {
    setRules((prev) => ({
      ...prev,
      groups: [...prev.groups, { id: `group-${Date.now()}`, operator: "and" as const, conditions: [] }],
    }));
  };

  const removeGroup = (groupId: string) => {
    setRules((prev) => ({ ...prev, groups: prev.groups.filter((g) => g.id !== groupId) }));
  };
  ...
  const addCondition = (groupId: string) => {
    setRules((prev) => ({
      ...
      groups: prev.groups.map((g) =>
        g.id === groupId
          ? { ...g, conditions: [...g.conditions, { id: `cond-${Date.now()}`, type: "prerequisite" as const, value: "", label: "" }] }
          : g
      ),
    }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const cleanedRules: BulkRules = { ...rules, groups: rules.groups.filter((g) => g.conditions.length > 0) };
  ...
  return (
    <NativeModal
      open={open}
      onClose={onClose}
      title={`Assign Rules - ${courseCode}`}
      description={`Configure rules for ${courseName} and select which programme-courses they apply to.`}
      icon={<GitBranch className="h-5 w-5 text-accent" />}
      className="max-w-[850px] max-h-[85vh]"
    >
      <form onSubmit={handleSubmit} className="space-y-5 max-h-[65vh] overflow-y-auto pr-1">
        {/* Step 1: Select Programmes */}
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label className="text-base font-semibold">1. Select Programme-Courses</Label>
          ...
          {programmes.length === 0 ? (
            ...
            <div className="border rounded-lg max-h-48 overflow-y-auto">
              {programmes.map((prog) => (
                <div
                  key={prog.id}
                  className={cn("flex items-center gap-3 p-3 hover:bg-muted/50 cursor-pointer transition-colors border-b last:border-b-0", selectedProgrammes.has(prog.id) && "bg-accent/5")}
                  onClick={() => toggleProgramme(prog.id)}
                >
                  <Checkbox checked={selectedProgrammes.has(prog.id)} onCheckedChange={() => toggleProgramme(prog.id)} />
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs px-1.5 py-0.5 rounded font-medium bg-muted">{degreeLabels[prog.degree]}</span>
                      <span className="font-medium">{prog.name}</span>
                      <span className="text-sm text-muted-foreground">({prog.code})</span>
                    </div>
                    <p className="text-xs text-muted-foreground">{prog.faculty}{prog.semester != null && ` • Sem ${prog.semester}`}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {selectedProgrammes.size > 0 && (
            <p className="text-xs text-accent font-medium">{selectedProgrammes.size} programme{selectedProgrammes.size !== 1 && "s"} selected</p>
          )}
        </div>

        {/* Step 2: Configure Rules */}
        <div className="space-y-3">
          <Label className="text-base font-semibold">2. Configure Rules</Label>

          {rules.groups.length > 1 && (
            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg border">
              <span className="text-sm font-medium">Connect groups with:</span>
              <Select value={rules.groupOperator} onValueChange={(v: "and" | "or") => setRules((prev) => ({ ...prev, groupOperator: v }))}>
          ...
          {rules.groups.length === 0 ? (
            <div className="text-center py-6 border border-dashed rounded-lg bg-muted/20">
              ...
              <p className="text-muted-foreground text-sm mb-3">No rule groups configured</p>
                ...
                <div key={group.id} className="space-y-2">
                  {gi > 0 && (
                    <div className="flex items-center gap-2 py-1">
                      ...
                      <span className={cn("px-3 py-0.5 text-xs font-bold rounded-full", rules.groupOperator === "and" ? "bg-accent/20 text-accent" : "bg-warning/20 text-warning")}>{rules.groupOperator.toUpperCase()}</span>
                      <div className="flex-1 border-t border-dashed" />
                    </div>
                  )}
                  <div className="border-2 rounded-lg overflow-hidden">
                    <div className="flex items-center justify-between p-2.5 bg-muted/30 border-b">
                      <div className="flex items-center gap-2">
                        ...
                        <Select value={group.operator} onValueChange={(v: "and" | "or") => setRules((prev) => ({ ...prev, groups: prev.groups.map((g) => g.id === group.id ? { ...g, operator: v } : g) }))}>
                      ...
                      <Button type="button" variant="ghost" size="sm" onClick={() => removeGroup(group.id)} className="text-destructive hover:text-destructive h-7">
                    ...
                    <div className="p-3 space-y-2">
                          ...
                          <div key={cond.id}>
                            {ci > 0 && (
                              <div className="flex justify-center py-1">
                                <span className={cn("px-2 py-0.5 text-xs font-bold rounded", group.operator === "and" ? "bg-accent/20 text-accent" : "bg-warning/20 text-warning")}>{group.operator.toUpperCase()}</span>
                              </div>
                            )}
                            <div className={cn("p-2.5 rounded-lg border-l-4", colors.bg, colors.border)}>
                              <div className="flex items-center gap-2">
                                <Select value={cond.type} onValueChange={(v: RuleCondition["type"]) => updateCondition(group.id, cond.id, { type: v, value: "" })}>
                                ...
                                {cond.type === "prerequisite" || cond.type === "corequisite" ? (
                                  <Select value={cond.value} onValueChange={(v) => updateCondition(group.id, cond.id, { value: v })}>
                                    <SelectTrigger className="h-8 flex-1 text-xs"><SelectValue placeholder="Select course..." /></SelectTrigger>
                                    <SelectContent>
                                      {availableCourses.filter((ac) => ac.code !== courseCode).map((ac) => (
                                        <SelectItem key={ac.code} value={ac.code}>{ac.code} - {ac.name}</SelectItem>
                                      ))}
                                    </SelectContent>
                                  </Select>
                                ) : (
                                  <Input type="number" min={1} className="h-8 w-24 text-xs" placeholder={cond.type === "ects_min" ? "e.g. 60" : "e.g. 3"} value={cond.value} onChange={(e) => updateCondition(group.id, cond.id, { value: e.target.value })} />
                                )}
                                <Button type="button" variant="ghost" size="sm" onClick={() => removeCondition(group.id, cond.id)} className="text-destructive h-7 w-7 p-0">
                                  <Trash2 className="h-3.5 w-3.5" />
                                </Button>
                              </div>
                            </div>
                      ...
                      <Button type="button" variant="ghost" size="sm" onClick={() => addCondition(group.id)} className="w-full text-xs">
        ...
        <div className="flex justify-end gap-2 pt-2 border-t">
          ...
          <Button type="submit" className="bg-accent hover:bg-accent/90 text-accent-foreground" disabled={selectedProgrammes.size === 0 || totalConditions === 0}>
            Apply to {selectedProgrammes.size} Programme{selectedProgrammes.size !== 1 && "s"}
          </Button>
        </div>
      </form>
    </NativeModal>
