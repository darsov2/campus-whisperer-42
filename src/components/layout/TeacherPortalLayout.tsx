...
function PortalSidebar({ teacherId }: { teacherId?: string }) {
  ...
  const items = [
    { label: "Dashboard", to: base, icon: LayoutDashboard, exact: true },
    { label: "Profile", to: `${base}/profile`, icon: User },
    { label: "My Courses", to: `${base}/courses`, icon: BookOpen },
    { label: "Student Lookup", to: `${base}/lookup`, icon: Users },
    { label: "Exam Sessions", to: `${base}/exam-sessions`, icon: CalendarRange },
    { label: "Grade Entry", to: `${base}/grades`, icon: ClipboardEdit },
    { label: "Signatures", to: `${base}/signatures`, icon: PenLine },
    { label: "Reports", to: `${base}/reports`, icon: BarChart3 },
    { label: "Email Students", to: `${base}/email`, icon: Mail },
  ];
        ...
        <Link to="/" className="flex items-center gap-2.5 px-2 py-2">
          ...
          {!collapsed && (
            <div className="leading-tight overflow-hidden">
              ...
              <p className="text-sm font-semibold text-sidebar-foreground truncate">Teacher Portal</p>
            </div>
          )}
        </Link>
      </SidebarHeader>
...
export function TeacherPortalLayout({ children }: Props) {
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 bg-[hsl(222_47%_11%)] text-primary-foreground border-b border-white/5 flex items-center justify-between px-4 gap-4">
            <div className="flex items-center gap-2">
              ...
              <div className="hidden md:block leading-tight ml-2">
                <p className="text-[11px] uppercase tracking-[0.18em] text-white/50">Faculty Portal</p>
                <p className="text-sm font-semibold">{t.title} {t.firstName} {t.lastName}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-2 pl-1">
                ...
                <div className="hidden sm:block leading-tight">
                  <p className="text-xs font-medium">{t.firstName} {t.lastName}</p>
                  <p className="text-[10px] text-white/50 font-mono">{t.staffId}</p>
                </div>
              </div>
              <Button variant="ghost" size="icon" className="h-8 w-8 text-white/70 hover:text-white hover:bg-white/10">
          ...
          <footer className="border-t bg-card">
            <div className="max-w-[1400px] mx-auto px-6 py-4 flex items-center justify-between text-xs text-muted-foreground">
              <p>© {new Date().getFullYear()} University Teacher Portal</p>
